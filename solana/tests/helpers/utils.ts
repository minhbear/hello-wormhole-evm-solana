...
import { CORE_BRIDGE_PID, MOCK_GUARDIANS } from "./constant";
...
export const programIdFromEnvVar = (envVar: string): PublicKey => {
...
export const boilerPlateReduction = (connection: Connection, defaultSigner: Signer) => {
  ...
  const requestAirdrop = async (account: PublicKey) =>
    connection.confirmTransaction(
      await connection.requestAirdrop(account, 5 * LAMPORTS_PER_SOL)
    );
